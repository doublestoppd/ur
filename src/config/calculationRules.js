...
(function (global) {
  ...
  var RULES = [
    ...
    rule({
      ...
      classification: C.OPERATIONAL,
      definition: 'Count of confirmed or probable internal OS -> IP status transitions whose transition moment (the inpatient admission, or failing that the observation discharge) falls inside the reporting period.',
      formula: 'count(transition links where fromService = OS, toService = IP, confidence in {Confirmed, Probable}, and the transition moment falls in the period)',
      inputs: ['TRANS_001'],
      inclusions: ['Accepted internal transitions whose status change happened inside the period - including conversions of observation stays that BEGAN before the period started.'],
      exclusions: ['Ambiguous candidates.', 'Missing successors.', 'Timing-only same-day service changes with no transition discharge code.', 'Conversions whose transition moment falls outside the period.'],
      nullHandling: 'Unlinked accounts are excluded and surfaced in the Transitions worksheet.',
      notes: 'Probable links are included in the count and separately identified in the Transitions worksheet. (v1.1: the count is anchored on the transition moment rather than the observation admission, so a stay that started before the period still counts when the conversion happened inside it.)',
    ...
    rule({
      ...
      formula: 'rate = acceptedOSIPTransitions / eligibleOSAccounts * 100, where eligibleOSAccounts = OS accounts whose stay overlaps the period (fully or partly) and that are neither open nor invalid',
    ...
    rule({
      ...
      classification: C.OPERATIONAL,
      definition: 'Count of accepted internal IP -> SB status transitions whose transition moment falls inside the reporting period.',
      formula: 'count(links where fromService = IP, toService = SB, confidence in {Confirmed, Probable}, and the transition moment falls in the period)',
      inputs: ['TRANS_001'],
      inclusions: ['Accepted internal transitions whose status change happened inside the period, even when the inpatient stay began before it.'],
      exclusions: ['Ambiguous or missing successors.', 'Transitions whose moment falls outside the period.'],
      nullHandling: 'Unlinked accounts are excluded and listed in diagnostics.',
      notes: '(v1.1: anchored on the transition moment - the successor admission, or failing that the prior discharge - rather than on the from-account admission.)',
    ...
    rule({
      ...
      classification: C.OPERATIONAL,
      definition: 'Count of accepted internal OS -> SB status transitions whose transition moment falls inside the reporting period, when present.',
      formula: 'count(links where fromService = OS, toService = SB, confidence in {Confirmed, Probable}, and the transition moment falls in the period)',
      inputs: ['TRANS_001'],
      inclusions: ['Accepted internal transitions whose status change happened inside the period.'],
      exclusions: ['Ambiguous or missing successors.', 'Transitions whose moment falls outside the period.'],
      nullHandling: 'Reported as zero when the pattern does not occur.',
      notes: 'Uncommon locally; reported so an unexpected occurrence is visible rather than absorbed. (v1.1: anchored on the transition moment.)',
      implementationKey: 'metrics.swingBed.transitions'
    }),
    ...
    rule({
      ...
      classification: C.OPERATIONAL,
      definition: 'Count of accepted internal SB -> IP status transitions whose transition moment falls inside the reporting period.',
      formula: 'count(links where fromService = SB, toService = IP, confidence in {Confirmed, Probable}, and the transition moment falls in the period)',
      inputs: ['TRANS_001'],
      inclusions: ['Accepted internal transitions whose status change happened inside the period, driven by the hospital-specific reading of discharge code V.'],
      exclusions: ['Ambiguous or missing successors.', 'Transitions whose moment falls outside the period.'],
      nullHandling: 'Unlinked accounts are excluded and listed in diagnostics.',
      sourceRefs: ['HOSP'],
      notes: 'Depends on the local use of code V. If the hospital changes that practice, update the discharge-code mapping rather than the metric. (v1.1: anchored on the transition moment.)',
      implementationKey: 'metrics.swingBed.transitions'
    }),
