...
(function (global) {
  ...
  var DQ_RULES = [
    ...
    dq('DQ_PERIOD_REVERSED', S.BLOCKING, 'Reporting period end precedes its start',
      'The chosen reporting-period end date is earlier than the start date.',
      'Every figure would silently report zero for an impossible period, so processing stops until the dates are corrected.'),
    ...
    dq('DQ_NEG_LOS', S.ERROR, 'Discharge earlier than admission',
      ...
      'The row is excluded from ALL figures - admissions, patients, LOS, occupancy, and review lists - until the dates are corrected. An inversion within the tolerance is instead clamped to a zero duration and kept, with a warning.'),
    ...
    dq('DQ_TRANS_UNCODED', S.WARNING, 'Possible uncoded transition',
      'A same-patient service change occurs close in time with no transition discharge code.',
      'No link is made on timing alone. Reported so a coding gap can be corrected at the source.'),
    dq('DQ_TRANS_UNMODELED', S.WARNING, 'Transition pair without a specific rule',
      'An internal transition was linked whose service pair (for example SB -> OS) has no named metric or review rule of its own.',
      'The link is kept and the episode stays continuous, but the pair appears in no named transition count. Verify that the discharge-code mapping is right for this pair; if the pattern is real and recurring, it needs a rule of its own.'),
    dq('DQ_OVERLAP_UNEXPLAINED', S.WARNING, 'Overlapping accounts for the same patient',
      'Two accounts for one patient overlap in time without a configured internal transition explaining it.',
      'Both rows remain in the data; occupancy metrics may double-count until the overlap is resolved.'),
    dq('DQ_ZERO_LOS', S.WARNING, 'Zero-length stay',
      'Admission and discharge are recorded at the same minute.',
      'The stay counts in LOS averages and the lowest LOS distribution band but not in the one-day-stay count, which requires a positive duration. Almost always a registration artifact; verify the times at the source.'),
    dq('DQ_POSTMORTEM_ADMIT', S.WARNING, 'Admission after a death-ended episode',
      'A later episode admits under the same derived Patient ID after an episode that ended in death.',
      'The pair is excluded from the readmission indicators: a deceased patient cannot be readmitted, so either two patients share a name and age or a discharge code is wrong. Both episodes are named for identity review.'),
